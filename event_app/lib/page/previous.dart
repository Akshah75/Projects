...
class PreviousEvent extends StatefulWidget {
  const PreviousEvent({Key? key}) : super(key: key);
...
class _PreviousEventState extends State<PreviousEvent> {
  ...
  void liveFilter() {
    ...
    results = _allUsers;
    // } else {
    results = _allUsers.where((user) => user["id"] >= 6).toList();
    print(results);
    ...
    return Scaffold(
      ...
      body: Padding(
        ...
        child: Column(
          children: [
            Expanded(
              child: foundlive.isNotEmpty
                  ? ListView.builder(
                      itemCount: foundlive.length,
                      itemBuilder: (context, index) => Card(
                        ...
                        child: ListTile(
                          leading: CircleAvatar(
                            radius: 30.0,
                            backgroundImage:
                                NetworkImage(foundlive[index]['image']),
                            backgroundColor: Colors.transparent,
                          ),
                          title: Text(foundlive[index]['name']),
                          subtitle: Text('${foundlive[index]["des"]}'),
                        ),
                      ),
                    )
                  : const Text(
