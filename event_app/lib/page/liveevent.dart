...
class LiveEvent extends StatefulWidget {
  const LiveEvent({Key? key}) : super(key: key);
    ...
    return Scaffold(
      ...
      body: Padding(
        ...
        child: Column(
          children: [
            Expanded(
              child: foundpre.isNotEmpty
                  ? ListView.builder(
                      itemCount: foundpre.length,
                      itemBuilder: (context, index) => Card(
                        ...
                        child: ListTile(
                          leading: CircleAvatar(
                            radius: 30.0,
                            backgroundImage:
                                NetworkImage(foundpre[index]['image']),
                            backgroundColor: Colors.transparent,
                          ),
                          title: Text(foundpre[index]['name']),
                          subtitle: Text('${foundpre[index]["des"]}'),
                        ),
                      ),
                    )
                  : const Text(
