...
import 'package:event_app/ApiModals/atheletApi.dart';
import 'package:event_app/ApiModals/modal_one.dart';
import 'package:event_app/page/liveevent.dart';
import 'package:event_app/page/previous.dart';
...
class _SearchSceenState extends State<SearchSceen> {
  ...
  List<Map<String, dynamic>> foundlive = [];
  List<Map<String, dynamic>> foundpre = [];
  @override
  initState() {
  ...
  //
  void liveFilter(String enteredKeyword) {
    List<Map<String, dynamic>> results = [];
    if (enteredKeyword.isEmpty) {
      // if the search field is empty or only contains white-space, we'll display all users
      results = _allUsers;
  ...
  void preFilter() {
    ...
    results = _allUsers;
    // } else {
    results = _allUsers.where((user) => user["id"] >= 3).toList();
    print(results);
    // we use the toLowerCase() method to make it case-insensitive
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text("Search your content")),
      body: Padding(
              ...
              children: [
                ElevatedButton(
                  onPressed: () async {
                    ...
                    // debugPrint(response);
                    // final user = Activity.fromJson(jsonDecode(response));
                    ...
                    final response = await BaseClient().post(detail);
                    print(response);
                  },
                ...
                ElevatedButton(
                  onPressed: () {
                    Navigator.of(context).push(
                      MaterialPageRoute(
                        builder: ((context) => const PreviousEvent()),
                      ),
                    );
                  },
